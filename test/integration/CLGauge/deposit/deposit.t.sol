...
contract DepositIntegrationConcreteTest is CLGaugeTest {
  ...
  function test_DepositBlockResetsOnRedeposit() public {
    // It should reset depositBlock on re-deposit, preventing penalty bypass
    uint256 tokenId = nftCallee.mintNewFullRangePositionForUserWith60TickSpacing(TOKEN_1, TOKEN_1, users.alice);

    nft.approve(address(gauge), tokenId);
    gauge.deposit({_lp: tokenId});

    uint256 firstDepositBlock = block.number;
    assertEq(gauge.depositBlock(tokenId), firstDepositBlock);
    ...
    assertEq(gauge.depositBlock(tokenId), block.number);
    assertNotEq(gauge.depositBlock(tokenId), firstDepositBlock);
  }
}
