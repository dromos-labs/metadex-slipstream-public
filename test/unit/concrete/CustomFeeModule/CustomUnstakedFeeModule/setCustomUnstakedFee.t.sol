...
contract SetCustomUnstakedFeeTest is CustomUnstakedFeeModuleTest {
  ...
  function test_RevertIf_NotManager() public {
    ...
    vm.expectRevert();
    customUnstakedFeeModule.setCustomFee({_pool: address(1), _fee: 5000});
  }
  ...
  function test_SetCustomFee() public {
    ...
    emit CustomFeeSet({pool: pool, fee: 5000});
    customUnstakedFeeModule.setCustomFee({_pool: pool, _fee: 5000});

    assertEqUint(customUnstakedFeeModule.customFee(pool), 5000);
    assertEqUint(customUnstakedFeeModule.getFee(pool), 5000);
    assertEqUint(poolFactory.getUnstakedFee(pool), 5000);
