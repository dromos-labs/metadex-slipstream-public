...
import {CLGauge, CLPoolSwapPartiallyStakedNoUnstakeFeeTest} from './CLPoolSwapPartiallyStakedNoUnstakeFee.t.sol';
...
contract MediumFeeToken0LiquidityOnlyPartiallyStakedTest is CLPoolSwapPartiallyStakedNoUnstakeFeeTest {
  function setUp() public override {
    ...
    stakedPositions.push(Position({tickLower: 0, tickUpper: 2000 * tickSpacing, liquidity: liquidity / 2}));

    unstakedPositions.push(Position({tickLower: 0, tickUpper: 2000 * tickSpacing, liquidity: liquidity / 2}));
    ...
    uint256 tokenId = nftCallee.mintNewCustomRangePositionForUserWithCustomTickSpacing(
      amount0 + 1, amount1 + 1, stakedPositions[0].tickLower, stakedPositions[0].tickUpper, tickSpacing, users.alice
    );
    nft.approve(address(gauge), tokenId);
    gauge.deposit(tokenId);

    nftCallee.mintNewCustomRangePositionForUserWithCustomTickSpacing(
      amount0 + 1, amount1 + 1, unstakedPositions[0].tickLower, unstakedPositions[0].tickUpper, tickSpacing, users.alice
    );
