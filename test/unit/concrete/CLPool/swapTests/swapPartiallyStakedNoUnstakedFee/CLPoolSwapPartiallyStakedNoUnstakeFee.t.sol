...
abstract contract CLPoolSwapPartiallyStakedNoUnstakeFeeTest is CLPoolSwapTests {
  ...
  function setUp() public virtual override {
    ...
    string memory path = string(
      abi.encodePacked(
        root, '/test/unit/concrete/CLPool/swapTests/swapPartiallyStakedNoUnstakedFee/swap_assert_partially_staked.json'
      )
    );
  ...
  function assertSwapData(
    ...
    string memory executionPriceStr
    ...
    assertApproxEqAbs(asd.feeGrowthGlobal0X128Delta, ss.feeGrowthGlobal0X128Delta, 340_282_366_920_938_463_464);
    assertApproxEqAbs(asd.feeGrowthGlobal1X128Delta, ss.feeGrowthGlobal1X128Delta, 340_282_366_920_938_463_464);
    ...
    if (asd.amount0Delta != 0) {
      ...
      assertEq(executionPrice, int256(stringToUint(executionPriceStr)));
    } else if (asd.amount1Delta == 0) {
      assertEq('NaN', executionPriceStr);
    } else {
      assertEq('-Infinity', executionPriceStr);
    }
  ...
  function burnStakedPosition() internal {
    ...
    for (uint256 i = 0; i < positionsLength; i++) {
      ...
      (uint128 liquidity,,,,) = CLPool(poolSetup.pool)
        .positions(
      ...
      CLPool(poolSetup.pool)
        .collect(
          users.alice, position.tickLower, position.tickUpper, type(uint128).max, type(uint128).max, poolSetup.gauge
        );
    }

    // sanity check
    skip(WEEK);
    // TODO: Update during integration testing
