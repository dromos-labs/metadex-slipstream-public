...
abstract contract CLPoolSwapTests is CLPoolTest {
  ...
  function assertSwapData(
    AssertSwapData memory asd,
    SuccessfulSwap memory ss,
  ...
  // Foundry 1.4+ auto-coerces quoted numeric strings in JSON to uint256, which breaks
  // `abi.decode(parseRaw(key), Struct)` for structs containing `string` fields whose JSON
  // value happens to look numeric (e.g. `"executionPrice": "665..."`). Some entries in our
  // fixtures also use truly non-numeric strings ("NaN", "-Infinity") for the same field.
  // To stay robust across the heterogeneity, we read each field via the typed parsers,
  // which coerce on the consumer side using a known target type rather than guessing.
  function _key(string memory base, string memory field) internal pure returns (string memory) {
    return string(abi.encodePacked(base, '.', field));
  }

  function _parseSuccessfulSwap(string memory assertKey) internal view returns (SuccessfulSwap memory s) {
    s.amount0Before = vm.parseJsonUint(jsonConstants, _key(assertKey, 'amount0Before'));
    s.amount0Delta = vm.parseJsonInt(jsonConstants, _key(assertKey, 'amount0Delta'));
    s.amount1Before = vm.parseJsonUint(jsonConstants, _key(assertKey, 'amount1Before'));
    s.amount1Delta = vm.parseJsonInt(jsonConstants, _key(assertKey, 'amount1Delta'));
    s.feeGrowthGlobal0X128Delta = vm.parseJsonUint(jsonConstants, _key(assertKey, 'feeGrowthGlobal0X128Delta'));
    s.feeGrowthGlobal1X128Delta = vm.parseJsonUint(jsonConstants, _key(assertKey, 'feeGrowthGlobal1X128Delta'));
    s.gaugeFeesToken0 = vm.parseJsonUint(jsonConstants, _key(assertKey, 'gaugeFeesToken0'));
    s.gaugeFeesToken1 = vm.parseJsonUint(jsonConstants, _key(assertKey, 'gaugeFeesToken1'));
    s.poolPriceAfterX96 = uint160(vm.parseJsonUint(jsonConstants, _key(assertKey, 'poolPriceAfterX96')));
    s.poolPriceBeforeX96 = uint160(vm.parseJsonUint(jsonConstants, _key(assertKey, 'poolPriceBeforeX96')));
    s.tickAfter = int24(vm.parseJsonInt(jsonConstants, _key(assertKey, 'tickAfter')));
    s.tickBefore = int24(vm.parseJsonInt(jsonConstants, _key(assertKey, 'tickBefore')));
  }

  function _parseFailedSwap(string memory assertKey) internal view returns (FailedSwap memory f) {
    f.poolBalance0 = vm.parseJsonUint(jsonConstants, _key(assertKey, 'poolBalance0'));
    f.poolBalance1 = vm.parseJsonUint(jsonConstants, _key(assertKey, 'poolBalance1'));
    // Fixture key is "poolPriceBefore" (no X96 suffix) for failed-swap entries.
    f.poolPriceBeforeX96 = uint160(vm.parseJsonUint(jsonConstants, _key(assertKey, 'poolPriceBefore')));
    f.swapError = vm.parseJsonString(jsonConstants, _key(assertKey, 'swapError'));
    f.tickBefore = int24(vm.parseJsonInt(jsonConstants, _key(assertKey, 'tickBefore')));
  }
  ...
  function runAsserts(
    ...
    if (swapFailed) {
      FailedSwap memory failedSwap = _parseFailedSwap(assertKey);
      ...
      SuccessfulSwap memory successfulSwap = _parseSuccessfulSwap(assertKey);
      ...
      assertSwapData(
        ...
        successfulSwap,
        vm.parseJsonString(jsonConstants, _key(assertKey, 'executionPrice'))
      );
    }
  }
  ...
  function assertSwapEvents(AssertSwapEvent memory ase) internal view {
    ...
    // Swap Event
    assertEq(
      ase.entries[swapEventIndex].topics[0], keccak256('Swap(address,address,int256,int256,uint160,uint128,int24)')
    );
