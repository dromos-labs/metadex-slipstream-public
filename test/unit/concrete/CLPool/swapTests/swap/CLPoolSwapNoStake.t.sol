...
abstract contract CLPoolSwapNoStakeTest is CLPoolSwapTests {
  function setUp() public virtual override {
    ...
    string memory path = string(abi.encodePacked(root, '/test/unit/concrete/CLPool/swapTests/swap/swap_assert.json'));
  ...
  function burnPosition() internal override {
    ...
    for (uint256 i = 0; i < positionsLength; i++) {
      ...
      CLPool(poolSetup.pool)
        .collect(users.alice, position.tickLower, position.tickUpper, type(uint128).max, type(uint128).max);
    }
  }
}
