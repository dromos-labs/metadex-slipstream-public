...
import {CLGauge, CLPoolSwapAllStakedNoUnstakeFeeTest} from './CLPoolSwapAllStakedNoUnstakeFee.t.sol';
...
contract MediumFee1to1PriceAdditionalLiquidityAroundCurrentPriceAllStakedTest is CLPoolSwapAllStakedNoUnstakeFeeTest {
  function setUp() public override {
    ...
    stakedPositions.push(Position({tickLower: getMinTick(tickSpacing), tickUpper: -tickSpacing, liquidity: liquidity}));

    stakedPositions.push(Position({tickLower: tickSpacing, tickUpper: getMaxTick(tickSpacing), liquidity: liquidity}));
    ...
    for (uint256 i = 0; i < positionsLength; i++) {
      ...
      uint256 tokenId = nftCallee.mintNewCustomRangePositionForUserWithCustomTickSpacing(
        amount0 + 1, amount1 + 1, stakedPositions[i].tickLower, stakedPositions[i].tickUpper, tickSpacing, users.alice
      );
