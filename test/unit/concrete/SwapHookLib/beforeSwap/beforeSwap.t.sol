...
contract UnitSwapHookLibBeforeSwap is UnitSwapHookLib {
  function test_WhenSwapHookIsAddressZero(uint24 _tickSpacingFee) external {
    ...
    uint24 _hookFee = swapHookLib.beforeSwap(address(0), _swapParams, factory, TICK_SPACING_60);
  ...
  modifier whenSwapHookIsntAddressZero(
    ...
    bytes memory _data = abi.encodeWithSelector(ISwapHook.beforeSwap.selector, _swapParams);
  ...
  function test_WhenCallFails(
    ...
    uint24 _hookFee = swapHookLib.beforeSwap(swapHook, _swapParams, factory, TICK_SPACING_60);
