...
contract CLFactoryTest is BaseFixture {
  function test_InitialState() public virtual {
    ...
    assertEq(poolFactory.discountRegistryManager(), users.discountRegistryManager);
    assertEq(poolFactory.clPoolTapeManager(), users.clPoolTapeManager);
    assertEq(poolFactory.clPoolTape(), address(0));
    assertEq(poolFactory.defaultSwapHook(), address(0));
    ...
    assertEqUint(poolFactory.tickSpacingToFee(TICK_SPACING_HIGH), 3000);
