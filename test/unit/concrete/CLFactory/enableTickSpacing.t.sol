...
contract EnableTickSpacingTest is CLFactoryTest {
  ...
  function test_RevertIf_NotOwner() public {
    ...
    vm.expectRevert();
    poolFactory.enableTickSpacing({tickSpacing: 250, fee: 5000});
  }

  function test_RevertIf_TickSpacingTooSmall() public {
    vm.expectRevert();
    poolFactory.enableTickSpacing({tickSpacing: 0, fee: 5000});
  }

  function test_RevertIf_TickSpacingTooLarge() public {
    vm.expectRevert();
    poolFactory.enableTickSpacing({tickSpacing: 16_834, fee: 5000});
  }
  ...
  function test_EnableTickSpacing() public {
    ...
    emit TickSpacingEnabled({tickSpacing: 250, fee: 5000});
    poolFactory.enableTickSpacing({tickSpacing: 250, fee: 5000});

    assertEqUint(poolFactory.tickSpacingToFee(250), 5000);
