...
contract CalculateFeesFuzzTest is CLPoolTest {
  ...
  function setUp() public override {
    ...
    deal({token: address(token0), to: users.alice, give: TOKEN_1 * 1_000_000});
    deal({token: address(token1), to: users.alice, give: TOKEN_1 * 1_000_000});
  }
  ...
  function assertFees(uint256 t0, uint256 t1, uint256 fg0, uint256 fg1) internal view {
    ...
    assertApproxEqAbs(pool.feeGrowthGlobal0X128(), fg0, 34_028_236_692_093_846_347);
    assertApproxEqAbs(pool.feeGrowthGlobal1X128(), fg1, 34_028_236_692_093_846_347);
  }
  ...
  function testFuzz_FeesCalculatedCorrectlyDuringSwapsFullRangePositionsIntermediaryUnstakeAndStake(
    ...
    uint256 tokenId = nftCallee.mintNewFullRangePositionForUserWith60TickSpacing(liquidity1, liquidity1, users.alice);

    uint256 tokenId2 = nftCallee.mintNewFullRangePositionForUserWith60TickSpacing(liquidity2, liquidity2, users.alice);
  ...
  function testFuzz_FeesCalculatedCorrectlyDuringSwapsFullRangePositionsIntermediaryUnstakeAndStake(
    ...
    fees.fee1 = FullMath.mulDivRoundingUp(fd.swapAmount1, 3000, 1e6);
    fees.fee2 = FullMath.mulDivRoundingUp(fd.swapAmount2, 3000, 1e6);
    fees.fee3 = FullMath.mulDivRoundingUp(fd.swapAmount3, 3000, 1e6);
    fees.fee4 = FullMath.mulDivRoundingUp(fd.swapAmount4, 3000, 1e6);
