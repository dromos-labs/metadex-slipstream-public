...
contract E2E_swap {
  ...
  function skip_some_time(uint256 seed) internal {
    ...
    uint256 timeToSkip = uint256(seed % 259_200) + 1;
    ...
    if (ProtocolTimeLibrary.epochStart(currentTime) != ProtocolTimeLibrary.epochStart(newTime)) {
      ...
      rewardToken.mint(address(voter), EMISSION_REWARD);
      hevm.prank(address(voter));
      rewardToken.approve(address(gauge), EMISSION_REWARD);
  ...
  function get_random_zeroForOne_priceLimit(int256 _amountSpecified) internal view returns (uint160 sqrtPriceLimitX96) {
    ...
    sqrtPriceLimitX96 = minimumPrice
      + uint160((uint256(_amountSpecified > 0 ? _amountSpecified : -_amountSpecified) % (currentPrice - minimumPrice)));
  }
  ...
  function get_random_oneForZero_priceLimit(int256 _amountSpecified) internal view returns (uint160 sqrtPriceLimitX96) {
    ...
    sqrtPriceLimitX96 = currentPrice
      + uint160((uint256(_amountSpecified > 0 ? _amountSpecified : -_amountSpecified) % (maximumPrice - currentPrice)));
  }
  ...
  function check_pool_reward_invariant(uint256 rewardGrowthBefore, uint256 rewardGrowthAfter) internal pure {
  ...
  function forgePoolParams(uint128 _seed) internal pure returns (PoolParams memory _poolParams) {
    ...
    _poolParams.maxTick = (int24(887_272) / _poolParams.tickSpacing) * _poolParams.tickSpacing;
  ...
  function testEchidna_emission_claiming_with_get_rewards(uint128 _amount, uint8 _position, uint256 _time) public {
    ...
    check_pool_reward_invariant(rewardGrowthBefore, rewardGrowthAfter);
    check_pool_staked_liquidity_invariant();
