...
import {MockCustomSwapFeeHook} from '../mocks/MockCustomSwapFeeHook.sol';
...
contract SetupCL {
  ...
  MockCustomSwapFeeHook public customSwapFeeModule;
  ...
  constructor(CoreTestERC20 _token0, CoreTestERC20 _token1) {
    ...
    rewardToken.mint(address(voter), 10_000_000_000e18);
    ...
    gaugeFactory = new CLGaugeFactory({
      ...
      _capConfig: CLGaugeFactory.CapConfig({
        defaultCap: 1e18, operatorMinCap: 1, operatorMaxCap: 1e18, maxMinStakeBlocks: 1 weeks
      })
    });
    ...
    customSwapFeeModule = new MockCustomSwapFeeHook(address(poolFactory));
    customUnstakedFeeModule = new CustomUnstakedFeeModule(address(poolFactory));
    poolFactory.setDefaultSwapHook(address(customSwapFeeModule));
  ...
  function createPool(int24 _tickSpacing, uint160 _startPrice) public {
    pool = CLPool(
      poolFactory.createPool({
        tokenA: address(token0), tokenB: address(token1), tickSpacing: _tickSpacing, sqrtPriceX96: _startPrice
      })
    );
    ...
    rewardToken.approve(address(gauge), 1_000_000_000e18);
...
contract CLMinter is ERC721Holder {
  ...
  function getReward(uint256 tokenId) public returns (uint256 collected) {
    ...
    tokenIds[0] = tokenId;
    gauge.claimEmissions(address(this), address(this), tokenIds);
