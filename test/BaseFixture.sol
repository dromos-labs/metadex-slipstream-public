...
import {ICLFactoryIndexation, MockCLFactoryIndexation} from './mocks/MockCLFactoryIndexation.sol';
...
import {CLPool, ICLPool} from 'contracts/core/CLPool.sol';
...
abstract contract BaseFixture is Test, Constants, Events, PoolUtils {
  ...
  MockCLFactoryIndexation public clFactoryIndexation;
  ...
  function setUp() public virtual {
    users = Users({
      ...
      discountRegistryManager: createUser('DiscountRegistryManager'),
      clPoolTapeManager: createUser('ClPoolTapeManager'),
      alice: createUser('Alice'),
      bob: createUser('Bob'),
      charlie: createUser('Charlie'),
      referral: createUser('Referral')
    });
    ...
    gaugeFactory.setPenaltyConfig(0, 0);
  ...
  function deployDependencies() public virtual {
    ...
    clFactoryIndexation = new MockCLFactoryIndexation();
  }
  ...
  function addRewardToGauge(address, address _gauge, uint256 _amount) internal {
    deal(address(rewardToken), address(voter), _amount);
    vm.startPrank(address(voter));
    // do not overwrite approvals if already set
    if (rewardToken.allowance(address(voter), _gauge) < _amount) {
