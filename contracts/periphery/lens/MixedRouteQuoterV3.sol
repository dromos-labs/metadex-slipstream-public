...
contract MixedRouteQuoterV3 is IMixedRouteQuoterV2, ICLSwapCallback, PeripheryImmutableState {
  ...
  int24 private constant volatileBitmask = 4_194_304;
  int24 private constant stableBitmask = 2_097_152;
  int24 private constant clBitmask = 1_048_576;
  int24 private constant cl2Bitmask = 524_288;
    ...
    while (true) {
      ...
      if (tickSpacing & volatileBitmask != 0) {
        amountIn = quoteExactInputSingleV2(
          QuoteExactInputSingleV2Params({tokenIn: tokenIn, tokenOut: tokenOut, amountIn: amountIn, stable: false})
        );
      } else if (tickSpacing & stableBitmask != 0) {
        amountIn = quoteExactInputSingleV2(
          QuoteExactInputSingleV2Params({tokenIn: tokenIn, tokenOut: tokenOut, amountIn: amountIn, stable: true})
        );
      } else {
        /// the outputs of prior swaps become the inputs to subsequent ones
        (uint256 _amountOut, uint160 _sqrtPriceX96After, uint32 _initializedTicksCrossed, uint256 _gasEstimate) = quoteExactInputSingleV3(
          QuoteExactInputSingleV3Params({
            tokenIn: tokenIn, tokenOut: tokenOut, tickSpacing: tickSpacing, amountIn: amountIn, sqrtPriceLimitX96: 0
          })
        );
    ...
    )
  {
    ...
    while (true) {
      ...
      (uint256 _amountIn, uint160 _sqrtPriceX96After, uint32 _initializedTicksCrossed, uint256 _gasEstimate) = quoteExactOutputSingleV3(
        QuoteExactOutputSingleV3Params({
          tokenIn: tokenIn, tokenOut: tokenOut, tickSpacing: tickSpacing, amountOut: amountOut, sqrtPriceLimitX96: 0
        })
      );
