...
contract QuoterV2 is IQuoterV2, ICLSwapCallback, PeripheryImmutableState {
    ...
    )
  {
    ...
    while (true) {
      ...
      (uint256 _amountOut, uint160 _sqrtPriceX96After, uint32 _initializedTicksCrossed, uint256 _gasEstimate) = quoteExactInputSingle(
    ...
    )
  {
    ...
    while (true) {
      ...
      (uint256 _amountIn, uint160 _sqrtPriceX96After, uint32 _initializedTicksCrossed, uint256 _gasEstimate) = quoteExactOutputSingle(
        QuoteExactOutputSingleParams({
          tokenIn: tokenIn, tokenOut: tokenOut, amount: amountOut, tickSpacing: tickSpacing, sqrtPriceLimitX96: 0
        })
      );
