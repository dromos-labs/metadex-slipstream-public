...
contract NonfungibleTokenPositionDescriptor is INonfungibleTokenPositionDescriptor {
  ...
  function tokenURI(
    ...
    ICLPool pool = ICLPool(
      PoolAddress.computeAddress(
        positionManager.factory(), PoolAddress.PoolKey({token0: token0, token1: token1, tickSpacing: tickSpacing})
      )
    );
    ...
    return string(
      abi.encodePacked(
        ...
        Base64.encode(
          bytes(abi.encodePacked('{', nameAndDescription, ', "image": "', 'data:image/svg+xml;base64,', image, '"}'))
        )
      )
    );
  }
  ...
  function generateSVG(
    ...
    (uint256 quoteTokensOwed, uint256 baseTokensOwed) = tokensOwed({
      positionManager: positionManager, tokenId: params.tokenId, _flipRatio: params.flipRatio
    });
