  ...
  SelfPermit
{
    ...
    )
  {
    ...
    (feeGrowthInside0LastX128, feeGrowthInside1LastX128) = _feeGrowthInsideLast(tokenId, position, poolKey);
    return (
      position.nonce,
      position.operator,
      poolKey.token0,
    ...
    returns (uint256 tokenId, uint128 liquidity, uint256 amount0, uint256 amount1)
  {
    if (params.sqrtPriceX96 != 0) {
      ICLFactory(factory)
        .createPool({
        tokenA: params.token0, tokenB: params.token1, tickSpacing: params.tickSpacing, sqrtPriceX96: params.sqrtPriceX96
      });
    }
    ...
    emit IncreaseLiquidity(tokenId, liquidity, amount0, amount1, params.tickLower, params.tickUpper);
  }
    ...
    returns (uint128 liquidity, uint256 amount0, uint256 amount1)
  {
    ...
    // emit before refundETH: the refund forwards all gas to msg.sender, which could reenter and burn the
    // position, zeroing position.tickLower/tickUpper before they are read
    emit MetadataUpdate(params.tokenId);
    emit IncreaseLiquidity(params.tokenId, liquidity, amount0, amount1, position.tickLower, position.tickUpper);
  ...
  /// @dev Returns the pool's current fee growth inside the range for a staked position, the stored one otherwise
  function _feeGrowthInsideLast(
    uint256 tokenId,
    Position memory position,
    PoolAddress.PoolKey memory poolKey
  ) private view returns (uint256, uint256) {
    ICLPool pool = ICLPool(PoolAddress.computeAddress(factory, poolKey));
    if (ownerOf(tokenId) == pool.gauge()) {
      return pool.getFeeGrowthInside(position.tickLower, position.tickUpper);
    }
