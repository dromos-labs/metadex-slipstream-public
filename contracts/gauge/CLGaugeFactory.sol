...
contract CLGaugeFactory is ICLGaugeFactory, AccessControl {
  ...
  /// @inheritdoc ICLGaugeFactory
  mapping(address => uint128) public override emissionCap;
  /// @inheritdoc ICLGaugeFactory
  ...
  function setEmissionCap(address _gauge, uint128 _cap) external override {
    ...
    if (_cap == 0 && ILeafVoter(leafVoter).hasRole(Roles.EMERGENCY_COUNCIL_ROLE, msg.sender)) {
      ILeafVoter(leafVoter).settleGauge(_gauge);
      _flushGaugeFees(_gauge);
      delete emissionCap[_gauge];
    ...
    if (hasRole(CAP_OPERATOR_ROLE, msg.sender)) {
      require(emissionCap[_gauge] != 0, 'NA');
  ...
  /// @inheritdoc ICLGaugeFactory
  function clearEmissionCap(address _gauge) external override {
    require(isGauge[_gauge], 'IG');
    require(ILeafVoter(leafVoter).hasRole(Roles.EMERGENCY_COUNCIL_ROLE, msg.sender), 'NA');

    ILeafVoter(leafVoter).settleGauge(_gauge);

    address votingRewardsManager = ICLGauge(_gauge).votingRewardsManager();
    try IVotingRewardsManager(votingRewardsManager).flushFees() {}
    catch {
  ...
  function _applyReferralParams(address _gauge, bytes calldata _params) internal {
    if (_params.length == 0 || uint8(_params[0]) != _PARAMS_REFERRAL) return;
  ...
  function _decodeReferral(bytes calldata _params) internal pure returns (address referral, uint256 share) {
    require(_params.length == _REFERRAL_PARAMS_LENGTH, 'IR');
