...
contract CLGauge is Gauge, ICLGauge, ERC721Holder {
  ...
  /// @inheritdoc ICLGauge
  ILeafVoter public immutable override voter;
  ...
  constructor(address _voter, address _nft, address _gaugeFactory) Gauge(_gaugeFactory) {
    voter = ILeafVoter(_voter);
  ...
  function initialize(
    ...
    require(msg.sender == address(gaugeFactory), 'NA');
  ...
  function settleGauge() external override returns (uint256 _delta) {
    ...
    uint256 _cumulativeRewardShare = voter.settleGauge(address(this));
  ...
  function forfeitRollover() external override nonReentrant {
    ...
    if (_rollover > 0) {
      voter.forfeitEmissions(_rollover.toUint128());
    }
  }
  ...
  function collectFees() external override nonReentrant returns (uint256, uint256) {
    ...
    if (gaugeFactory.emissionCap(address(this)) == 0) return (0, 0);
  ...
  function earned(address _account, uint256[] memory _tokenIds) public view override returns (uint256) {
    ...
    if (_stakedLiquidity > 0) {
      uint256 _cumulativeRewardShare = voter.projectedCumulativeRewardShare(address(this));
      uint256 _lastCumulativeRewardShare = lastCumulativeRewardShare;
    ...
    (address _referral, uint256 _referralShare) = gaugeFactory.referralConfig(address(this));
    ICLGaugeFactory.PenaltyConfig memory _penaltyConfig = gaugeFactory.effectivePenaltyConfig(address(this));
  ...
  function _withdraw(uint256 tokenId, address account, address recipient) internal override {
    ...
    _tokenIds[0] = tokenId;
    _claimPositions(account, _tokenIds, recipient);
   ...
   * @dev Assumes `_tokenIds` are staked by `_account`
  ...
  function _claimPositions(address _account, uint256[] memory _tokenIds, address _recipient) internal {
    ...
    if (_totalForfeitedAmount > 0) {
      voter.forfeitEmissions(_totalForfeitedAmount.toUint128());
    }

    (address[] memory _recipients, uint128[] memory _amounts) =
      _buildMintArrays(_recipient, _totalLpAmount, _referral, _totalReferralAmount);

    if (_amounts.length > 0) {
      voter.mintEmissions(_recipients, _amounts);
    }
  }
  ...
  function _computePositionEmissions(
    ...
    if (_reward > 0) {
      ...
      if (_referralAmount > 0) {
        ...
        emit ReferralPaid(_account, _tokenId, _referral, _referralAmount);
      }
  ...
  function _isActivated() internal view returns (bool _activated) {
    return voter.isActivated(address(this));
  }
}
