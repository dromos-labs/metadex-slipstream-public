...
interface ISwapHook {
  ...
  /// @param sqrtPriceX96 Pre-swap sqrt price of the pol.
  /// @param tick Pre-swap tick of the pool.
  struct SwapParams {
    address caller;
    address recipient;
    bool zeroForOne;
  ...
  /// @param secondsPerLiquidityCumulativeX128 The post-swap value of the seconds per liquidity accumulator, computed only if an initialized tick is crossed.
  ...
  /// @param _swapParams Swap context and final pool state.
  /// @param _afterSwapParams Post-swap execution state and oracle/liquidity context.
  /// @return _fee Post-swap fee to apply to the non-exact input/output part of the swap.
  function afterSwap(
