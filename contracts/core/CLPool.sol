...
contract CLPool is ICLPool {
  ...
  function _updateRewardsGrowthGlobal(uint256 _rewardsToDistribute, uint48 _lastUpdated) internal {
    if (stakedLiquidity > 0) {
      rewardGrowthGlobalX128 += FullMath.mulDiv(_rewardsToDistribute, FixedPoint128.Q128, stakedLiquidity);
    } else {
  ...
  function _settleToBlock() internal {
    ...
    if (_timestamp == lastUpdated) return;

    address _gauge = gauge;
    if (_gauge == address(0)) return;

    uint256 _rewardsToDistribute = ICLGauge(_gauge).settleGauge();
    _updateRewardsGrowthGlobal(_rewardsToDistribute, _timestamp);
  }
  ...
  function setGaugeAndPositionManager(address _gauge, address _nft) external override lock onlyGaugeFactory {
    ...
    uint256 _rewardsToDistribute = ICLGauge(_gauge).settleGauge();
    _updateRewardsGrowthGlobal(_rewardsToDistribute, uint48(block.timestamp));
  }
}
